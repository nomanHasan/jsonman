...
export class Parser {
  ...
  static partial(input: string, options?: ParseOptions): { success: boolean; data?: JSONValue; isPartial: boolean; completedString?: string; error?: Error } {
    ...
    if (normalResult.success && normalResult.data !== undefined) {
      return {
        success: true,
    ...
    if (!completed.endsWith('}') && completed.includes('{')) {
      ...
      const quotePairs = completed.split('"').length - 1;
      if (quotePairs % 2 === 1) {
        // Unclosed string, close it first
        completed += '"';
      }
    ...
    if (completedResult.success && completedResult.data !== undefined) {
      return {
        success: true,
    ...
    return {
      success: false,
      isPartial: false,
      error: normalResult.error ? (normalResult.error as unknown as Error) : new Error('Unknown parsing error')
    };
  }
